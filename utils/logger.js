    ...
    try {
      ...
      logFiles.forEach(file => {
        ...
        try {
          ...
          // Only delete .log files that are older than 5 days
          if (stats.isFile() && 
              file.endsWith('.log') && 
              stats.mtime < fiveDaysAgo) {
  ...
  logCurrentDiskUsage() {
    try {
      ...
      this.info('Current log disk usage', {
        ...
        averageFileSize: fileCount > 0 ? `${(totalSize / fileCount / 1024).toFixed(2)}KB` : '0KB'
      });
      ...
      this.debug('Failed to calculate log disk usage', { error: error.message });
    }
  }
}
